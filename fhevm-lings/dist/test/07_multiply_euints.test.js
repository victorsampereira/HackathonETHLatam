...
describe("HomomorphicMultiply", function () {
    it("should implement the multiply function with a return statement", async function () {
        const sourceCode = fs.readFileSync(path.join(__dirname, "../exercises/07_multiply_euints.sol"), "utf-8");
        ...
        if (!hasImplementation) {
            throw new Error("A função 'multiply' precisa retornar o produto de 'a' e 'b'! " +
                "Dica: Use 'return a * b' ou 'return FHE.mul(a, b)'");
        }
        let contractFactory;
        try {
            contractFactory = await hardhat_1.ethers.getContractFactory("HomomorphicMultiply");
        }
        catch (e) {
            throw new Error("A compilação falhou. Verifique sua implementação. " + e.message);
        }
        const contract = await contractFactory.deploy();
        (0, chai_1.expect)(contract.multiply).to.be.a('function');
    });
});
