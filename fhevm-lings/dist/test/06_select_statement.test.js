...
describe("ConfidentialLogic", function () {
    it("should implement conditionalSelect with a return statement", async function () {
        ...
        const sourceCode = fs.readFileSync(path.join(__dirname, "../exercises/06_select_statement.sol"), "utf-8");
        ...
        if (!hasImplementation) {
            throw new Error("A função 'conditionalSelect' precisa retornar o resultado da seleção condicional! " +
                "Dica: Use 'return FHE.select(condition, ifTrueValue, ifFalseValue)'");
        }
        // Tenta compilar e fazer deploy
        let contractFactory;
        try {
            contractFactory = await hardhat_1.ethers.getContractFactory("ConfidentialLogic");
        }
        catch (e) {
            throw new Error("A compilação falhou. " + e.message);
        }
        const contract = await contractFactory.deploy();
        (0, chai_1.expect)(contract.conditionalSelect).to.be.a('function');
    });
});
