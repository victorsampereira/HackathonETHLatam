...
describe("ConfidentialComparison", function () {
    it("should implement isGreaterOrEqual with a return statement", async function () {
        ...
        const sourceCode = fs.readFileSync(path.join(__dirname, "../exercises/05_comparison.sol"), "utf-8");
        ...
        if (!hasImplementation) {
            throw new Error("A função 'isGreaterOrEqual' precisa retornar a comparação entre 'a' e 'b'! " +
                "Dica: Use 'return FHE.ge(a, b)' ou 'return FHE.gte(a, b)'");
        }
        // Tenta compilar e fazer deploy
        let contractFactory;
        try {
            contractFactory = await hardhat_1.ethers.getContractFactory("ConfidentialComparison");
        }
        catch (e) {
            throw new Error("A compilação falhou. " + e.message);
        }
        const contract = await contractFactory.deploy();
        (0, chai_1.expect)(contract.isGreaterOrEqual).to.be.a('function');
    });
});
