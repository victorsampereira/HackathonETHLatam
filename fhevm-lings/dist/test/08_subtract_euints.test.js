...
describe("HomomorphicSubtract", function () {
    it("should implement the subtract function with a return statement", async function () {
        const sourceCode = fs.readFileSync(path.join(__dirname, "../exercises/08_subtract_euints.sol"), "utf-8");
        ...
        if (!hasImplementation) {
            throw new Error("A função 'subtract' precisa retornar a diferença entre 'a' e 'b'! " +
                "Dica: Use 'return a - b' ou 'return FHE.sub(a, b)'");
        }
        let contractFactory;
        try {
            contractFactory = await hardhat_1.ethers.getContractFactory("HomomorphicSubtract");
        }
        catch (e) {
            throw new Error("A compilação falhou. Verifique sua implementação. " + e.message);
        }
        const contract = await contractFactory.deploy();
        (0, chai_1.expect)(contract.subtract).to.be.a('function');
    });
});
